...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tagSelect.style.minHeight = '100px';
        ...
        if (!empty($_GET['tag_filter'])) {
            $filter_tags = is_array($_GET['tag_filter']) ? $_GET['tag_filter'] : array($_GET['tag_filter']);
            foreach ($filter_tags as $tag_id) {
                $term = get_term($tag_id, 'post_tag');
                if ($term) {
                    $active_filters[] = 'Tag: ' . $term->name;
                }
            }
        }
        ...
        <div class="row gy-4 g-md-4">
            ...
            $tag_filter = isset($_GET['tag_filter']) ? array_map('intval', (array)$_GET['tag_filter']) : array();
